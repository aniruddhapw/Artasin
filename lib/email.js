...
export async function sendPersonalizedEmails(messages) {
  ...
  const provider = getEmailProvider();
  await provider.sendEach(messages);
...
export async function sendBulkEmail(recipients, { subject, html }) {
  ...
  try {
    const provider = getEmailProvider();
    await provider.sendBatch(recipients, { subject, html });
  } catch (error) {
    console.error(`Bulk email failed (${recipients.length} recipients):`, error.message);
  }
}
